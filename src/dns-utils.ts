...
export function decodeDNSName(nameBytes: Bytes): string {
    ...
    log.info("Decoded DNS name: {}", [decodedName]);
  ...
  switch (type) {
    ...
    case 2: // NS (Name Server)
      return decodeLabels(rdata as Array<u8>); // Decode domain name from labels

    case 6: // MX (Mail Exchange)
      let preference: u16 = (rdata[0] << 8) | rdata[1]; // MX preference
      let exchange = decodeLabels(rdata.slice(2) as Array<u8>); // MX exchange domain
      return `Preference: ${preference}, Exchange: ${exchange}`;

    case 43: // DS (Delegation Signer)
      return `Key Tag: ${(rdata[0] << 8) | rdata[1]}, Algorithm: ${rdata[2]}, Digest Type: ${rdata[3]}, Digest: ${rdata.slice(4).map<string>((byte: u8): string => byte.toString(16).padStart(2, "0")).join('')}`;

    case 52: // TLSA (Transport Layer Security Authentication)
      return `Usage: ${rdata[0]}, Selector: ${rdata[1]}, Matching Type: ${rdata[2]}, Certificate: ${rdata.slice(3).map<string>((byte: u8): string => byte.toString(16).padStart(2, "0")).join('')}`;

    case 16: // TXT (Text)
      return rdata.slice(1).map<string>((byte: u8): string => String.fromCharCode(byte)).join('');

    default:
      return 'Unsupported or unknown record type';
  }
}

  
  
  // Helper function to decode labels from a DNS name (used for CNAME, NS, MX)
  function decodeLabels(rdata: number[]): string {
    let index = 0;
    let labels: string[] = [];
  
    while (index < rdata.length) {
      let labelLength = rdata[index];
      if (labelLength === 0) break; // End of domain name
      labels.push(rdata.slice(index + 1, index + 1 + labelLength).map(byte => String.fromCharCode(byte)).join(''));
      index += labelLength + 1;
    }
